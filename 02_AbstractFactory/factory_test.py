...
class FactoryTest(unittest.TestCase):
    ...
    def test_large_factory(self):
        ...
        self.assertEqual(factory.createButton().getHtml(),   "<button size = 5 >  </button>")
        self.assertEqual(factory.createHeader().getHtml(),   "<h size = 5 >  </h>")
        self.assertEqual(factory.createImage().getHtml(),    "<image size = 5 >  </image>")
        self.assertEqual(factory.createText().getHtml(),     "<p size = 5 >  </p>")

    def test_medium_factory(self):
        factory = MediumFactory()
        self.assertEqual(factory.createButton().getHtml(),   "<button size = 3 >  </button>")
        self.assertEqual(factory.createHeader().getHtml(),   "<h size = 3 >  </h>")
        self.assertEqual(factory.createImage().getHtml(),    "<image size = 3 >  </image>")
        self.assertEqual(factory.createText().getHtml(),     "<p size = 3 >  </p>")

    def test_small_factory(self):
        factory = SmallFactory()
        self.assertEqual(factory.createButton().getHtml(),   "<button size = 1 >  </button>")
        self.assertEqual(factory.createHeader().getHtml(),   "<h size = 1 >  </h>")
